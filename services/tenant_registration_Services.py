...
from models.enums import NecessityEnum
...
async def create_tenant(
    ...
    user = Users(
        **tenant_create.model_dump(),
        ...
        tenant_id=tenant_id,
        user_role="tenant",
        password=tenant_create.password,
    )
    db.add(tenant)
    db.add(user)
    db.commit()
    ...
    tenant_necessity = (
        db.query(TenantRegistration)
        .filter(TenantRegistration.hostel_id == hostel_id)
        .filter(TenantRegistration.tenant_id == tenant_id)
        .first()
    )
...
async def add_student_details(
    db: Session,
    tenant_student_create: TenantStudentCreate,
) -> TenantStudent:
...
async def add_employee_details(
    db: Session,
    tenant_employee_create: TenantEmployeeCreate,
    tenant_id: str,
    hostel_id: str,
) -> TenantEmployee:
...
def add_self_employed(
    db: Session,
    tenant_self_employee_create: TenantSelfEmployedCreate,
) -> TenantSelfEmployed:
